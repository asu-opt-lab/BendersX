...
@testset verbose = true "CFLP Auto Decompose - Callback" begin
    ...
    for i in instances
        @testset "Instance: p$i" begin
            ...
            @testset "Classical oracle" begin
                @info "solving CFLP p$i - auto_decompose - callback - classical"
            ...
            @testset "Disjunctive oracle" begin
                ...
                for strengthened in [true], add_benders_cuts_to_master in [true], reuse_dcglp in [true], lift in [true], p in [1.0], disjunctive_cut_append_rule in [AllDisjunctiveCuts()], adjust_t_to_fx in [true]
                    ...
                    @testset "strgthnd $strengthened; benders2master $add_benders_cuts_to_master; reuse $reuse_dcglp; lift $lift; p $p; dcut_append $disjunctive_cut_append_rule; adjust_t_to_fx $adjust_t_to_fx" begin
                        ...
                        @testset "NoSeq" begin
                            @info "solving CFLP p$i - auto_decompose - callback - disjunctive - no seq - strgthnd $strengthened; benders2master $add_benders_cuts_to_master reuse $reuse_dcglp lift $lift p $p dcut_append $disjunctive_cut_append_rule adjust_t_to_fx $adjust_t_to_fx"
                        ...
                        @testset "Seq" begin
                            @info "solving CFLP p$i - auto_decompose - callback - disjunctive - seq - strgthnd $strengthened; benders2master $add_benders_cuts_to_master reuse $reuse_dcglp lift $lift p $p dcut_append $disjunctive_cut_append_rule adjust_t_to_fx $adjust_t_to_fx"
                            master = Master(data_for_mip; solver_param = master_solver_param)
                            update_model!(master, data_for_mip)
                        ...
                        @testset "SeqInOut" begin
                            @info "solving CFLP p$i - auto_decompose - callback - disjunctive - seqinout - strgthnd $strengthened; benders2master $add_benders_cuts_to_master reuse $reuse_dcglp lift $lift p $p dcut_append $disjunctive_cut_append_rule adjust_t_to_fx $adjust_t_to_fx"
                            master = Master(data_for_mip; solver_param = master_solver_param)
                            update_model!(master, data_for_mip)
