...
OUTPUT_DIR="experiments_snip/milp/snipno3_private"
...
cp -r src "${OUTPUT_DIR}"
cp scripts/snip_milp.jl "${OUTPUT_DIR}"
...
for instance in "${instances[@]}"; do
    for snipno in "${snipnos[@]}"; do
        for budget in "${budgets[@]}"; do
            ...
            echo "julia --project=. scripts/snip_milp.jl --snip_instance ${instance} --snip_no ${snipno} --snip_budget ${budget} --output_dir ${OUTPUT_DIR}" >> "${JOBSCRIPT_FILE}"
