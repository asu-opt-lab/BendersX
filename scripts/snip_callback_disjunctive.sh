...
OUTPUT_DIR="experiments_snip/disjunctive/th7_rootol9/private/snipno4_th7_rootol9_LATT1.0TT_f500_extrabudget"
...
cp -r src "${OUTPUT_DIR}"
cp scripts/snip_callback_disjunctive.jl "${OUTPUT_DIR}"
...
for instance in "${instances[@]}"; do
    for snipno in "${snipnos[@]}"; do
        for budget in "${budgets[@]}"; do
            ...
            echo "julia --project=. scripts/snip_callback_disjunctive.jl --snip_instance ${instance} --snip_no ${snipno} --snip_budget ${budget} --output_dir ${OUTPUT_DIR}" >> "${JOBSCRIPT_FILE}"
