...
function generate_cuts(oracle::UFLKnapsackOracle, x_value::Vector{Float64}, t_value::Vector{Float64}; tol_normalize = 1.0, time_limit = 3600.0)
    ...
    for j in 1:oracle.J
        ...
        if oracle.obj_values[j] >= t_value[j]/tol_normalize * (1 + oracle.oracle_param.rtol)
    ...
    is_in_L = sum(oracle.obj_values) >= sum(t_value)/tol_normalize * (1 + oracle.oracle_param.rtol) ? false : true
